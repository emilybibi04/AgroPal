...
import { 
  ...
  ShoppingBag,
  PlusCircle,
  Flame // Icono para Bomberos
} from 'lucide-react';

// --- Componentes UI Reutilizables ---

const Card = ({ children, className = "", onClick }) => (
  <div onClick={onClick} className={`bg-white rounded-xl shadow-md p-4 ${className} ${onClick ? 'cursor-pointer active:scale-95 transition-transform' : ''}`}>
    {children}
  </div>
);
...
const INITIAL_WORKERS = [
  { 
    ...
    accessCode: '123456', 
    tasks: [
      { id: 't1', title: 'Poda de Formación', type: 'urgente', time: '08:00 AM', deadline: '2023-11-25', completed: false, observation: '' },
      { id: 't2', title: 'Registro Fotográfico', type: 'normal', time: '02:00 PM', deadline: '2023-11-26', completed: true, observation: 'Se encontraron manchas negras en hojas inferiores.' }
    ]
  },
  { 
    id: 2, 
    ...
    accessCode: '654321',
    tasks: [
      { id: 't3', title: 'Fertilización', type: 'normal', time: '07:00 AM', deadline: '2023-11-30', completed: false, observation: '' }
    ]
  },
  {
    id: 3,
...
export default function App() {
  ...
  const [showCreateTask, setShowCreateTask] = useState(false); // Estado para modal de crear tarea
  ...
  // Eliminamos 'weather' de forecastInputs manuales, ahora se basa en simulación
  const [forecastInputs, setForecastInputs] = useState({ hectares: 2, plantAge: 5 });
  const [forecast, setForecast] = useState({ yield: 0, risk: 0, riskLabel: 'Bajo', income: 0 });
  ...
  // --- Funciones CSV Import (Plug & Play) ---
  ...
  const processCSV = (csvText) => {
    ...
    if (newTasks.length > 0) {
      const updatedWorkers = workers.map(w => {
        if (w.id === currentWorkerId || (userRole === 'admin' && w.id === 1)) {
  ...
  const handleCreateTask = (e) => {
    ...
    // Si es admin, toma el asignado, si es productor, se asigna a sí mismo (currentWorkerId)
    // Nota: Para la demo, el Producer usa el ID 1 o currentWorkerId. 
    // Si es Admin, necesitamos el ID del select.
    let targetWorkerId = userRole === 'admin' ? parseInt(e.target.assignee.value) : currentWorkerId;
    
    // Fallback por si currentWorkerId no está seteado correctamente en modo producer demo
    if (userRole === 'producer' && !targetWorkerId) targetWorkerId = 1; 
  ...
  // --- Lógica Forecasting & Clima ---
  useEffect(() => {
    // Simulación de Clima (Random o fijo para demo)
    const isRainy = Math.random() > 0.5;
    
    // Cálculo simplificado
    let baseYield = forecastInputs.hectares * 500;
    let ageFactor = forecastInputs.plantAge < 5 ? 0.7 : 1.2;
    // Usamos el clima simulado en lugar de input manual
    let weatherFactor = isRainy ? 1.1 : 0.9; 
    let diseaseRisk = isRainy ? 45 : 15;

    const finalYield = Math.round(baseYield * ageFactor * weatherFactor);
    setForecast({
      yield: finalYield,
      risk: diseaseRisk,
      riskLabel: diseaseRisk > 70 ? 'Crítico' : (diseaseRisk > 30 ? 'Medio' : 'Bajo'),
      income: (finalYield * 3.5).toLocaleString()
    });
    ...
    const timer = setTimeout(() => {
      setWeatherStationData({
        temp: isRainy ? 24 : 32,
        humidity: isRainy ? 88 : 60,
        rain: isRainy ? 15 : 0,
        name: "Estación INAMHI - Zona Sur"
      });
    }, 1500); 
    return () => clearTimeout(timer);

  }, [forecastInputs]); // Se recalculan si cambian las hectáreas/edad

  // --- LÓGICA CALCULADORA FERTILIZACIÓN ---
  ...
  const calculateFertilization = () => {
    ...
    if (calcMode === 'analysis') {
      const nVal = parseFloat(soilAnalysis.n) || 0;
      const pVal = parseFloat(soilAnalysis.p) || 0;
      const kVal = parseFloat(soilAnalysis.k) || 0;
      needs.n = Math.max(0, 120 - (nVal * 0.5));
      needs.p = Math.max(0, 60 - (pVal * 0.4));
      needs.k = Math.max(0, 140 - (kVal * 0.6));
      method = 'Basado en Análisis de Suelo';
    } else {
      needs = { n: 80, p: 40, k: 80 }; 
      if (quizAnswers.leaves === 'yellow') needs.n += 40; 
      if (quizAnswers.growth === 'poor') needs.p += 20; 
      if (quizAnswers.soil === 'sandy') { needs.n *= 1.2; needs.k *= 1.2; }
    ...
    const dapKgPerHa = needs.p / 0.46;
    const nFromDapPerHa = dapKgPerHa * 0.18;
    const remainingNPerHa = Math.max(0, needs.n - nFromDapPerHa);
    const ureaKgPerHa = remainingNPerHa / 0.46;
    const kclKgPerHa = needs.k / 0.60;

    const totalNutrientsKg = (needs.n + needs.p + needs.k) * hectares;
    const commercialMixKg = totalNutrientsKg * 2.5; 
    const commercialSacks = roundToQuarter(commercialMixKg / sackWeight);

    setFertResult({
      urea: Math.round(ureaKgPerHa),
      dap: Math.round(dapKgPerHa),
      kcl: Math.round(kclKgPerHa),
      ...
      commercialSacks: commercialSacks,
      hectaresUsed: hectares,
      method
    });
  };

  const resetCalculator = () => {
    setCalcMode(null); setFertResult(null); setFertTab('components'); setSoilAnalysis({ n: '', p: '', k: '' }); setQuizAnswers({ leaves: '', soil: '', growth: '' });
  };
  ...
  const submitTaskObservation = () => {
    ...
    const updatedWorkers = workers.map(w => {
      // Admin updatea al worker[0] en demo, worker/producer se updatean a si mismos
      const targetId = (userRole === 'admin') ? 1 : currentWorkerId;
      if (w.id === targetId) { 
        return { ...w, tasks: w.tasks.map(t => t.id === taskToComplete.id ? { ...t, completed: true, observation: observationText } : t) };
      }
      return w;
    });
    setWorkers(updatedWorkers); setTaskToComplete(null); setObservationText('');
  };
  ...
  const renderAdminDashboard = () => (
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <Card className="bg-red-50 border border-red-100"><div className="flex gap-3"><AlertTriangle className="text-red-600 shrink-0" /><div><h3 className="font-bold text-red-800">Alertas Activas</h3><p className="text-sm text-red-700 mt-1">Se detectaron 2 reportes críticos hoy.</p></div></div></Card>
      <Card onClick={() => setShowFertilizerCalc(true)} className="bg-blue-50 border border-blue-200 flex justify-between items-center cursor-pointer hover:bg-blue-100 transition-colors">
        <div className="flex items-center gap-3"><div className="bg-blue-100 p-3 rounded-full text-blue-600"><FlaskConical size={24} /></div><div><h3 className="font-bold text-blue-900">Calculadora de Fertilización</h3><p className="text-sm text-blue-700">Calcular dosis por hectárea</p></div></div><ChevronRight className="text-blue-400" />
      </Card>
      {userRole !== 'tech' && (
        <div>
                        ...
                        {pendingTasks.map(t => {
                          ...
                          return (
                            <li key={t.id} className="flex justify-between text-xs items-center">
                              <span className="flex items-center gap-1 text-amber-900"><span className="w-1 h-1 rounded-full bg-amber-500 shrink-0"></span>{t.title}</span>
                              <span className={`text-[10px] font-bold ${isOverdue ? 'text-red-600' : 'text-gray-500'}`}>{isOverdue ? 'Vencida' : t.deadline}</span>
                            </li>
                          );
                        })}
                      </ul>
      ...
      <div className="space-y-6 animate-fade-in pb-20">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-800">
              {userRole === 'admin' ? 'Planificación Global' : (userRole === 'producer' ? 'Mi Plantación' : 'Mi Hectárea')}
            </h2>
            <p className="text-gray-500 text-sm">{userRole === 'admin' ? 'Asignación de Tareas' : 'Tareas de hoy'}</p>
          </div>
          <div className="flex gap-2">
            {(userRole === 'admin' || userRole === 'producer') && (
              <>
                <input type="file" ref={fileInputRef} onChange={handleFileUpload} accept=".csv" className="hidden" />
                <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-1 bg-green-100 text-green-700 text-xs px-2 py-2 rounded-lg font-bold hover:bg-green-200 transition-colors"><FileSpreadsheet size={16} /></button>
                <button onClick={() => setShowCreateTask(true)} className="flex items-center gap-1 bg-blue-600 text-white text-xs px-3 py-2 rounded-lg font-bold hover:bg-blue-700 transition-colors shadow-md"><PlusCircle size={16} /> Crear Tarea</button>
              </>
            )}
          </div>
        </div>

        <Card className="bg-gradient-to-r from-green-800 to-green-600 text-white border-none shadow-lg"><div className="flex justify-between items-start mb-4"><div><p className="text-green-100 text-xs uppercase tracking-wider">Estado del Cultivo</p><h3 className="text-2xl font-bold mt-1">Floración</h3></div><Sun className="text-yellow-300 w-8 h-8 animate-pulse" /></div><div className="w-full bg-green-900/30 h-2 rounded-full mb-2"><div className="bg-yellow-400 h-2 rounded-full" style={{ width: '30%' }}></div></div><p className="text-xs text-green-100">Día 45 / 150</p></Card>
        
        <div>
          <h3 className="font-bold text-gray-700 mb-3">Lista de Tareas</h3>
          {myData.tasks.map((task) => {
              ...
              <Card key={task.id} className={`flex justify-between items-center mb-3 border-l-4 transition-colors ${task.completed ? 'border-gray-400 bg-gray-50' : (isOverdue ? 'border-red-500 bg-red-50' : 'border-green-600')}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-xs flex items-center gap-1 ${isOverdue ? 'text-red-600 font-bold' : 'text-gray-500'}`}>
                      <Clock size={10} /> {isOverdue ? `Venció: ${task.deadline}` : `Vence: ${task.deadline}`}
                    </p>
                  </div>
                </div>
          ...
          {myData.tasks.length === 0 && <p className="text-gray-500 text-sm text-center py-4">No tienes tareas asignadas.</p>}
        </div>
        ...
        <Modal isOpen={showCreateTask} onClose={() => setShowCreateTask(false)} title="Nueva Tarea">
          <form onSubmit={handleCreateTask} className="space-y-4">
            <div>
              <label className="text-xs font-bold text-gray-500 mb-1 block">Título de la Tarea</label>
              <input name="title" type="text" className="w-full p-2 border border-gray-300 rounded-lg" placeholder="Ej. Poda de mantenimiento" required />
            </div>
            
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="text-xs font-bold text-gray-500 mb-1 block">Fecha Límite</label>
                <input name="deadline" type="date" className="w-full p-2 border border-gray-300 rounded-lg" required />
              </div>
              <div className="flex-1">
                <label className="text-xs font-bold text-gray-500 mb-1 block">Prioridad</label>
                <select name="type" className="w-full p-2 border border-gray-300 rounded-lg">
            ...
            {userRole === 'admin' && (
              <div>
                <label className="text-xs font-bold text-gray-500 mb-1 block">Asignar a:</label>
                <select name="assignee" className="w-full p-2 border border-gray-300 rounded-lg">
                  {workers.filter(w => w.role !== 'admin').map(w => (
                    <option key={w.id} value={w.id}>{w.name} ({w.role === 'tech' ? 'Técnico' : 'Jornalero'})</option>
                  ))}
                </select>
              </div>
            )}
  ...
  const renderSupport = () => {
    // Buscar el técnico asociado en la lista de trabajadores
    ...
    return (
      <div className="space-y-6 animate-fade-in pb-20">
        ...
        <div className="space-y-3">
          ...
          <Card className="flex justify-between items-center border border-red-100 bg-red-50">
            <div className="flex items-center gap-3">
              <div className="bg-red-100 p-2 rounded-full text-red-600"><Activity size={20} /></div>
              <div><h4 className="font-bold text-gray-800">Emergencia Médica</h4><p className="text-xs text-gray-600">Ambulancia Rural</p></div>
            </div>
            <Button variant="danger" className="py-1 px-3 text-sm h-auto">Llamar 911</Button>
          </Card>
          
          {/* BOTÓN BOMBEROS NUEVO */}
          <Card className="flex justify-between items-center border border-orange-100 bg-orange-50">
            <div className="flex items-center gap-3">
              <div className="bg-orange-100 p-2 rounded-full text-orange-600"><Flame size={20} /></div>
              <div><h4 className="font-bold text-gray-800">Bomberos</h4><p className="text-xs text-gray-600">Incendios / Rescate</p></div>
            </div>
            <Button className="bg-orange-600 hover:bg-orange-700 text-white py-1 px-3 text-sm h-auto border-none">Llamar 102</Button>
          </Card>

          {/* Contacto Técnico Dinámico */}
          <Card className="flex justify-between items-center border border-gray-100 hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-3">
              <div className="bg-blue-100 p-2 rounded-full text-blue-700"><UserCog size={20} /></div>
              <div>
                <h4 className="font-semibold text-gray-800">{tech ? tech.name : 'Sin Técnico Asignado'}</h4>
                <p className="text-xs text-gray-500">Soporte Técnico Plantación</p>
              </div>
            </div>
            {tech && (
              <button className="bg-gray-100 p-2 rounded-full text-green-700 hover:bg-green-200"><Phone size={18} /></button>
            )}
          </Card>
  ...
  const renderForecasting = () => (
    <div className="space-y-6 animate-fade-in pb-20">
      <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
        <TrendingUp className="text-green-700" /> Proyecciones
      </h2>
      ...
      <Card className="border border-blue-100 bg-gradient-to-br from-blue-50 to-white">
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-sm font-bold text-blue-800 flex items-center gap-2">
            <Radio size={16} className={weatherStationData ? "text-green-500 animate-pulse" : "text-gray-400"} /> 
            {weatherStationData ? "Estación Conectada" : "Buscando Señal..."}
          </h3>
          <span className="text-[10px] bg-white px-2 py-1 rounded border border-blue-100 text-gray-500">Actualizado: Hace 1m</span>
        </div>
          ...
          <div className="animate-fade-in">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-white p-2 rounded-xl shadow-sm">
                ...
                <span className="block text-lg font-bold text-gray-800">{weatherStationData.temp}°C</span>
                <span className="text-[10px] text-gray-400">Temp</span>
              </div>
              <div className="bg-white p-2 rounded-xl shadow-sm">
                <Droplets size={20} className="mx-auto text-blue-500 mb-1"/>
                <span className="block text-lg font-bold text-gray-800">{weatherStationData.humidity}%</span>
                <span className="text-[10px] text-gray-400">Humedad</span>
              </div>
              <div className="bg-white p-2 rounded-xl shadow-sm">
                <CloudRain size={20} className="mx-auto text-gray-600 mb-1"/>
                <span className="block text-lg font-bold text-gray-800">{weatherStationData.rain}mm</span>
                <span className="text-[10px] text-gray-400">Lluvia</span>
              </div>
            </div>
          </div>
        ) : (
          <div className="h-24 flex flex-col items-center justify-center text-gray-400">
            <Wifi size={32} className="animate-ping mb-2 opacity-50"/>
            <p className="text-xs">Sincronizando con estación cercana...</p>
          </div>
        )}
      </Card>

      {/* Solo mostramos Inputs de Finca, no de clima */}
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card className="text-center"><p className="text-xs text-gray-500 mb-1">Rendimiento Est.</p><p className="text-2xl font-bold text-gray-800">{forecast.yield} kg</p></Card>
        <Card className="text-center"><p className="text-xs text-gray-500 mb-1">Ingreso Proy.</p><p className="text-2xl font-bold text-green-700">${forecast.income}</p></Card>
      </div>
      <Card className={`${forecast.risk > 50 ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'} border`}><div className="flex justify-between items-center mb-2"><h3 className={`font-bold ${forecast.risk > 50 ? 'text-red-800' : 'text-green-800'}`}>Riesgo Fitosanitario</h3><span className="text-xs font-bold px-2 py-1 rounded-full bg-white/50">{forecast.risk}%</span></div><div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden"><div className={`h-full ${forecast.risk > 50 ? 'bg-red-500' : 'bg-green-500'}`} style={{ width: `${forecast.risk}%` }}></div></div></Card>
    </div>
  );

  const renderChatbot = () => (<div className="flex flex-col h-[calc(100vh-140px)]"><div className="bg-white p-3 rounded-xl shadow-sm mb-3"><h2 className="text-lg font-bold text-gray-800">AgroIA</h2><p className="text-xs text-gray-500">{isOnline ? 'Conectado a la nube' : 'Modo Offline (Local)'}</p></div><div className="flex-1 overflow-y-auto space-y-3 p-2 bg-gray-50 rounded-xl mb-3">{messages.map((m, i) => (<div key={i} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] p-3 rounded-xl text-sm ${m.sender === 'user' ? 'bg-green-700 text-white rounded-tr-none' : 'bg-white border text-gray-800 rounded-tl-none'}`}>{m.text}</div></div>))}</div><div className="flex gap-2"><input value={inputMsg} onChange={(e) => setInputMsg(e.target.value)} placeholder="Escribe o sube foto..." className="flex-1 p-3 rounded-full border border-gray-300 text-sm outline-none focus:border-green-500" /><button onClick={() => {if(!inputMsg) return; setMessages([...messages, {sender:'user', text: inputMsg}, {sender:'bot', text: 'Analizando solicitud...'}]); setInputMsg('');}} className="bg-green-700 text-white p-3 rounded-full hover:bg-green-800"><ChevronRight size={20}/></button></div></div>);
        ...
        {onboardingStep === 'welcome' && (
          <div className="z-10 text-center w-full space-y-8 animate-fade-in p-4">
            <div className="flex flex-col items-center gap-2 mb-8">
              <div className="bg-white p-4 rounded-full shadow-xl"><Leaf className="text-green-800 w-12 h-12" /></div>
  ...
  return (
    <div className="max-w-md mx-auto h-screen bg-gray-50 flex flex-col font-sans relative overflow-hidden shadow-2xl">
      ...
      <div className="bg-white border-t border-gray-200 px-4 py-2 flex justify-between items-center pb-6 md:pb-2">
        ...
        {(userRole === 'admin' || userRole === 'producer') && <button onClick={() => setActiveTab('forecast')} className={`flex flex-col items-center gap-1 ${activeTab === 'forecast' ? 'text-green-700' : 'text-gray-400'}`}><TrendingUp size={24} /> <span className="text-[10px]">$$$</span></button>}
      ...
      <Modal isOpen={showAddWorker || !!newUserCreated} onClose={() => {setShowAddWorker(false); setNewUserCreated(null);}} title={newUserCreated ? "Usuario Creado" : "Nuevo Usuario"}>
        {newUserCreated ? (
          ...
          <form onSubmit={handleAddWorker} className="space-y-4">
            ...
            <Modal
        isOpen={showSoilGuide}
        onClose={() => setShowSoilGuide(false)}
        title="Guía de Suelos"
      >
        <div className="space-y-4">
          <div className="bg-blue-50 p-3 rounded-lg flex gap-3 items-start border border-blue-100">
            <Info className="text-blue-600 shrink-0 mt-1" size={20} />
            <p className="text-xs text-blue-800 leading-relaxed">
              <strong>Prueba del tacto:</strong> toma tierra húmeda y trata de formar
              una cinta entre el pulgar e índice.
            </p>
          </div>

          <div className="space-y-3 text-xs text-gray-700">
            <div className="bg-white p-2 rounded-lg border border-gray-200">
              <strong>Suelo arenoso:</strong> se deshace fácil, no forma cinta.
              Drena rápido pero pierde nutrientes.
            </div>
            <div className="bg-white p-2 rounded-lg border border-gray-200">
              <strong>Suelo franco:</strong> forma una cinta corta antes de romperse.
              Es el punto medio ideal.
            </div>
            <div className="bg-white p-2 rounded-lg border border-gray-200">
              <strong>Suelo arcilloso:</strong> forma una cinta larga y plástica.
              Retiene agua pero se encharca con facilidad.
            </div>
          </div>
